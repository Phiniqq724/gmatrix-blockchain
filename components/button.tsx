...
export const Button = ({ children, onClick, disabled }: ButtonProps) => {
  return (
    <button onClick={onClick} disabled={disabled} className={`flex w-full items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-blue-600 duration-500 group`}>
      ...
      <span className="flex-1 ms-3 whitespace-nowrap text-lg text-start">{children}</span>
    </button>
  );
};
